...
class InventoryObject:
    def __init__(
        ...
        self.assettype = (
            assettype  # what type of asset is it? (router, switch, all-in-one)
        )
        self.assignedto = assignedto  # what person / dept is asset assigned to?
        self.assetlocation = (
            assetlocation  # where is the asset? e.g. GOVH, HLTH (enum to pick from)
        )
        ...
        self.status = (
            status  # is item in production? bool. False = not in use, True = In use
        )
