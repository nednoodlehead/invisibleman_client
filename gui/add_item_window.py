...
class GenericAddJsonWindow(QWidget):
     def __init__(self, target: str, parent_window):
          ...
          self.json_years_frame.setStyleSheet(u"")
          ...
          self.json_write_to_button.setText(u"Update")
          self.json_exit_button.setText(u"Exit")
          self.json_new_value_label.setText(u"Value")
          self.json_conditional_years_text.setText("")
          self.json_conditional_years_label.setText(u"Years until expiry")
          self.json_add_new_value_button.setText(u"Add new value")
          font1 = QFont()
          font1.setPointSize(8)
          self.json_add_new_value_button.setFont(font1)
     ...
     def make_x_button(self): # func to make the x and delete button for the little psuedo-menu
     ...
     def write_changes(self):
          ...
          if self.target == "Category":
               ...
               data = []
               for index in length:
                    val = self.json_value_layout.itemAt(index).widget().text()  # get the data from the combobox
                    data.append(val)
          replace_json_target(self.target, data)  # replace the json with the new data
          self.parent_window.refresh_combobox(self.target)  # send signal to main window to refresh comboboxes
          self.close()                   
